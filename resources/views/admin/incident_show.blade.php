...
<div class="incident-details-container" style="width:100%;margin:0;background:linear-gradient(135deg,#fff 0%,#f8f9ff 100%);border-radius:24px;box-shadow:0 12px 40px rgba(91,48,126,0.10);padding:0;overflow:hidden;">
    ...
    <div class="incident-details-content" style="padding:2.5rem 2rem 0 2rem;">
        <div class="incident-info-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:2rem 2.5rem;margin-bottom:2.5rem;">
            ...
            <div class="incident-info-item">
                <div class="incident-info-label">Client</div>
                <div class="incident-info-value">{{ $incident->user->name ?? 'Non défini' }}</div>
            </div>
            <div class="incident-info-item">
                <div class="incident-info-label">Date de création</div>
                <div class="incident-info-value">{{ $incident->creationdate->format('d/m/Y H:i') ?? 'Non définie' }}</div>
            </div>
        </div>
        @if($incident->notes)
        ...
        <div class="status-update-section" style="margin-bottom:2.5rem;background:linear-gradient(135deg,#fff 0%,#f8f9ff 100%);border-radius:16px;box-shadow:0 8px 32px rgba(91,48,126,0.08);border:1px solid rgba(91,48,126,0.08);overflow:hidden;">
            <div class="status-update-header" style="background:linear-gradient(135deg, #5B307E, #7B4A9E);color:#fff;padding:1.2rem 1.5rem;font-weight:600;font-size:1.1rem;">Modifier le statut</div>
            <div class="status-update-content" style="padding:1.5rem;">
                <form action="{{ route('admin.incidents.updateStatus', $incident->id) }}" method="POST" class="status-form" style="display:flex;gap:1.2rem;align-items:flex-end;">
                    @csrf
                    @method('PATCH')
                    <div style="flex: 1;">
                    ...
                    <button type="submit" class="status-submit-btn" style="background:#5B307E;color:#fff;padding:0.7rem 2rem;border:none;border-radius:8px;font-weight:600;cursor:pointer;height:fit-content;">Mettre à jour</button>
                </form>
            </div>
        </div>
                        ...
                        <div style="margin-bottom:1rem;display:flex;{{ $isCurrentUser ? 'justify-content:flex-end;' : 'justify-content:flex-start;' }}">
                            <div style="max-width:70%;background:#f7f7fa;color:#333;border-radius:12px;padding:0.8rem 1rem;box-shadow:0 2px 4px rgba(91,48,126,0.04);">
                                <div style="font-size:0.95rem;margin-bottom:0.3rem;font-weight:600;color:#5B307E;">
                                    {{ $comment->user->name }}
                                    @if($isAdmin)
                                        <span style="background:#FFC107;color:#5B307E;padding:0.2rem 0.5rem;border-radius:10px;font-size:0.7rem;margin-left:0.5rem;font-weight:600;">Admin</span>
                                    @elseif($isEmployee)
            ...
            <div style="border-top:1px solid #eee;padding:1.5rem;background:#fafafa;">
                <form action="{{ route('comments.store', $incident->id) }}" method="POST">
                    ...
                    <div style="display:flex;gap:0.5rem;align-items:flex-end;">
                        ...
                        <button type="submit" class="btn chat-send-btn" style="padding:0.8rem 1.5rem;border-radius:8px;background:#5B307E;color:#fff;border:none;cursor:pointer;font-weight:600;height:fit-content;display:flex;align-items:center;">Envoyer</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="delete-section" style="margin-bottom:2.5rem;background:linear-gradient(135deg,#fff 0%,#fff5f5 100%);border-radius:16px;box-shadow:0 8px 32px rgba(220,38,38,0.08);border:1px solid rgba(220,38,38,0.1);overflow:hidden;">
            <div class="delete-header" style="background:linear-gradient(135deg, #DC2626, #EF4444);color:#fff;padding:1.2rem 1.5rem;font-weight:600;font-size:1.1rem;">Supprimer l'incident</div>
            <div class="delete-content" style="padding:1.5rem;">
                <p style="color:#666;margin-bottom:1rem;">Cette action est irréversible. Tous les fichiers, commentaires et notifications associés seront également supprimés.</p>
                <form action="{{ route('admin.incidents.delete', $incident->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cet incident ? Cette action est irréversible.');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="delete-btn" style="background:#DC2626;color:#fff;padding:0.8rem 2rem;border:none;border-radius:8px;font-weight:600;cursor:pointer;transition:background 0.2s;">Supprimer l'incident</button>
                </form>
            </div>
        </div>
    </div>
