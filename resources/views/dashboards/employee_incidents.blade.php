...
@section('title', 'Tous les Incidents')
...
<div class="dashboard-header client-incidents-title-up">
    <h1 style="margin:0 0 0.7rem 0;font-size:2rem;color:#5B307E;font-weight:700;">Tous les Incidents</h1>
    <p style="color:#666;margin-top:0;margin-bottom:1.5rem;">Gérez et suivez tous les incidents signalés</p>
</div>
<!-- Search/Filter Bar -->
...
<div>
    @if ($incidents->isEmpty())
        <div class="card" style="text-align:center;padding:3rem;color:#666;">
            ...
            <p style="margin:0;">Aucun incident n'a été signalé pour le moment.</p>
        </div>
    @else
                ...
                <div class="incident-card" style="background:#fff;border-radius:18px;box-shadow:0 8px 32px rgba(91,48,126,0.12);border:1.5px solid #e2e8f0;padding:2.2rem 1.2rem 2rem 1.2rem;display:flex;flex-direction:column;gap:1.2rem;position:relative;transition:box-shadow 0.2s;min-height:180px;width:100%;border-top:none;">
                    ...
                    <div class="incident-content" style="padding:2.2em 0 0.5em 0;flex:1;display:flex;flex-direction:column;justify-content:space-between;height:100%;">
                        ...
                        <div class="incident-meta" style="display:flex;flex-wrap:wrap;gap:2em 2.5em;margin-bottom:1.2em;align-items:center;">
                            <div style="font-size:1.05rem;color:#764ba2;"><span style="font-weight:600;">Client:</span> {{ $incident->user->name ?? 'Client non défini' }}</div>
                            <div style="font-size:1.05rem;color:#1976d2;"><span style="font-weight:600;">Projet:</span> {{ $incident->projet->nom ?? 'Projet non défini' }}</div>
                            <div style="font-size:1.05rem;color:#888;"><span style="font-weight:600;">Priorité:</span> {{ $incident->priority ?? 'Normale' }}</div>
                            <div style="font-size:1.05rem;color:#888;"><span style="font-weight:600;">Date:</span> {{ $incident->creationdate->format('d/m/Y') }}</div>
                        </div>
                        <div style="margin-top:auto;">
                            <a href="{{ route('employee.incidents.show', $incident->id) }}" class="incident-action-btn" style="background:#5B307E;color:#fff;padding:0.8em 2em;border-radius:999px;font-weight:600;text-decoration:none;display:inline-block;font-size:1.08rem;">Voir détails</a>
                        </div>
                    </div>
                </div>
            @endforeach
